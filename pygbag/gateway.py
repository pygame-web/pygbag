...
class BaseSOCKS5Handler:
    def __init__(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter,
                 host: Union[str, ipaddress.IPv4Address, ipaddress.IPv6Address], port: int):
...
class SimpleSOCKS5Handler(BaseSOCKS5Handler):
    async def do_TCP_open(self):
        ...
        try:
            ...
            return self.close()
        await SimpleSOCKS5Handler._bridge(self.client_reader, self.client_writer,
                                          server_reader, server_writer)

    async def do_TCP_bind(self):
        async def on_conn(server_reader: asyncio.StreamReader, server_writer: asyncio.StreamWriter):
            await self._bridge(self.client_reader, self.client_writer,
                               server_reader, server_writer)
            srv.close()
            logging.info('TCP UNBOUND %s:%s == %s:%s',
                         self.client_host, self.client_port,
                         self.dest_host_str(), self.dest_port)

        logging.info('TCP BIND %s:%s == %s:%s',
                     self.client_host, self.client_port,
                     self.dest_host_str(), self.dest_port)
        try:
            ...
            logging.info('TCP BOUND %s:%s == %s:%s',
                         self.client_host, self.client_port,
                         self.dest_host_str(), self.dest_port)
            self.response_status(SOCKS5Status.OK)
        except OSError as e:
    ...
    @staticmethod
    async def _bridge(active_reader: asyncio.StreamReader, active_writer: asyncio.StreamWriter,
                      passive_reader: asyncio.StreamReader, passive_writer: asyncio.StreamWriter):
        """
        Cut the passive reader/writer if the active ones disconnect.
        """
        async def pipe(from_: asyncio.StreamReader, to: asyncio.StreamWriter, cut: asyncio.StreamWriter):
            try:
